...
export function BarraClasificacion({
  ...
  return (
    <div className="px-4 py-3 border-b border-stone-200 dark:border-white/8 bg-stone-50 dark:bg-white/3 shrink-0 space-y-2.5">
      {/* Mensaje informativo */}
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        <span className="text-[10px] text-stone-400 dark:text-stone-600 uppercase tracking-wide font-semibold shrink-0">
          Tipo:
        </span>
        ...
        {clasificacionActual !== "NINGUNA" && (
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-medium border text-stone-500 dark:text-stone-500 border-stone-200 dark:border-stone-700/50 bg-stone-100 dark:bg-stone-800/30 hover:bg-stone-200 dark:hover:bg-stone-700/40 transition-all disabled:opacity-60"
          >
            <X className="h-3 w-3" />
            Quitar etiqueta
          </button>
        ...
        {accionando && <Loader2 className="h-3 w-3 text-stone-400 dark:text-stone-500 animate-spin shrink-0" />}
      </div>

      {/* Descripción de la clasificación activa */}
      {opcionActiva && (
        <p className="text-[10px] text-stone-400 dark:text-stone-500 leading-snug pl-1">
