...
export async function POST(req: NextRequest) {
  try {
    ...
    const plantillaId = form.get("plantillaId");
    const instanciaId = (form.get("instanciaId") as string | null) ?? "default";

    if (!(file instanceof File))
      return NextResponse.json({ error: "Archivo inválido" }, { status: 400 });
    if (!plantillaId || typeof plantillaId !== "string")
      return NextResponse.json({ error: "plantillaId requerido" }, { status: 400 });
    ...
    const relDir = path.join("uploads", instanciaId, "plantilla");
    const absDir = path.join(process.cwd(), "public", relDir);
    await fs.mkdir(absDir, { recursive: true });

    const filename = `${plantillaId}.webp`;
