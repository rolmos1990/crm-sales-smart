...
interface UploadImagenPlantillaProps {
...
export function UploadImagenPlantilla({
  plantillaId,
  instanciaId,
  value,
  onChange,
}: UploadImagenPlantillaProps) {
  const inputRef = useRef<HTMLInputElement>(null);
  const [subiendo, setSubiendo] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [modoUrl, setModoUrl] = useState(!plantillaId);
  const [dragging, setDragging] = useState(false);
  const [cacheBust, setCacheBust] = useState(0);

  const subirArchivo = async (file: File) => {
    if (!plantillaId) {
      setError("Guarda la plantilla primero para poder subir una imagen.");
      return;
    }
    setError(null);
    setSubiendo(true);
    ...
    fd.append("file", file);
    fd.append("plantillaId", plantillaId);
    fd.append("instanciaId", instanciaId);

    try {
      const res = await fetch("/api/upload/plantilla", { method: "POST", body: fd });
