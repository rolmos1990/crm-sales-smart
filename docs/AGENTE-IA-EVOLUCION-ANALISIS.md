...
> Estado: **las 10 specs (`009`-`018`) están implementadas, testeadas y mergeadas a `main`** (2026-09-01) — el plan completo de evolución del agente de IA está cerrado. Historial de implementación (orden real, no numérico: 009→010→012→011→013→015→016→014→017→018):
...
> - ✅ `016-niveles-autonomia-automatizacion` — implementada (2026-09-01). Gate real (`decidirAutonomia`) insertado en `generar-respuesta-ia.suscriptor.ts` antes de `enviarMensaje`: sin ninguna fila de `AutonomiaIntencionConfig` para el agente, el comportamiento es exactamente el de antes (nunca clasifica, siempre envía — FR-004/SC-001). Con configuración, clasifica la intención (`TareaIA.CLASIFICACION`) y aplica 4 niveles (`AUTO_REPLY_SAFE_INTENTS`/`CONDITIONAL_AUTOMATION`/`SUGGESTION_ONLY`/`HUMAN_ONLY`) con el más severo entre categorías candidatas. Bandeja de revisión nueva en `/crm/inbox/pendientes-ia`. Siembra de la clasificación inicial (research.md Decisión 1) al crear un agente nuevo — no vía `prisma/seed.ts` (ver `specs/016-.../tasks.md`, "Notas de implementación").
> - ✅ `014-conversaciones-piloto-ejemplos-relevantes` — implementada (2026-09-01). `ConversacionPiloto` (positiva/negativa, anonimizada de forma determinística — bug real de orden de sustitución detectado y corregido por el propio test) → `RecomendacionComportamiento` (vía análisis con el gateway de IA, nunca auto-aplicado — FR-008) → `EjemploPrompt`, recuperado por `IRecuperadorEjemplos` (2-4 ejemplos por coincidencia de etiquetas, nunca rellena con irrelevantes). Reemplaza el placeholder de la capa 9 de `013` (`producirCapaEjemplosPiloto`), ahora con efecto real en el prompt generado. "Marcar como piloto" implementado como selector propio (no se tocó `InboxLayout`, componente central del Inbox); "convertir en regla" simplificado a copiar/pegar hacia `009` en vez de una confirmación diferida entre specs (ver `specs/014-.../tasks.md`, "Notas de implementación").
> - ✅ `017-aprendizaje-supervisado-auditoria` — implementada (2026-09-01). `RespuestaPendienteRevision` (de `016`) extendida con `estrategiaUtilizadaId`/`ejemplosUtilizadosIds`/`herramientasEjecutadas`/`confianza`/`motivoTransferencia`/`usoIAId`/`productoIdentificadoId`; nuevo estado `ENVIADA_AUTOMATICAMENTE` — el camino `ENVIAR` del gate de `016` ahora también registra traza completa, no solo el camino `PENDIENTE`. `ensamblarYPersistirRegistro` (FR-010, nunca propaga error) unifica ambos caminos. `EvaluacionRespuestaIA` nueva (BUENA/NECESITA_MEJORA, sin límite de una por respuesta). `ejecutarAnalisisPiloto` (`014`) gana `incluirCorreccionesRecientes` como insumo adicional, sin ninguna ruta nueva de escritura hacia `AgenteIAConfig`. Vista de auditoría consolidada en la tab IA.
> - ✅ `018-simulador-agente` — implementada (2026-09-01), **última spec del plan de 10 — cierre completo**. `ContextoTool.modoSimulacion?: boolean` propagado a las 6 tools que escriben (`crear_cotizacion`, `crear_pedido`, `agregar_productos_oportunidad`, `transferir_a_humano`, `actualizar_info_contacto`, `agregar_etiqueta_contacto`): con el flag activo, cada una devuelve la misma previsualización calculada sin tocar Prisma (verificado por test explícito por tool — SC-002/SC-003). `SimuladorService.ejecutar` orquesta `009`-`017` con un perfil de cliente 100% simulado (nunca un contacto real) y, opcionalmente, la versión borrador (`009`) en vez de la publicada — sin persistir nada en las tablas de auditoría de `017`. Nuevas pestañas "Simulador" y "Conocimiento" en la ficha del agente. Historia 4 (navegación de 10 secciones) acotada a los dos puntos de navegación ya establecidos por la arquitectura real (instancia vs. por-agente) en vez de una reestructuración plana forzada (ver `specs/018-.../tasks.md`, "Notas de implementación").

**Plan completo cerrado**: las 10 specs (`009`–`018`) están implementadas, testeadas (235 tests unitarios en verde) y en producción (`main`). Ver cada `specs/0XX-.../tasks.md` para el detalle de adaptaciones y limitaciones documentadas de cada una.
